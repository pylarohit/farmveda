...
export default function RentAndHirePage() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col xl:flex-row items-start justify-between gap-6 pb-10">

        {/* ── LEFT COLUMN (Widgets) ── */}
        <div className="w-full xl:w-[340px] xl:flex-shrink-0 flex flex-col gap-6 xl:sticky xl:top-[104px] z-10">
          ...
          {/* Activity Box */}
          <div className="bg-white dark:bg-[#0A0E1A] rounded-[32px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-xl transition-all border border-slate-100 dark:border-[#1E293B] flex flex-col flex-1 min-h-[300px] group/box">
            <h3 className="font-sora font-black text-slate-900 dark:text-white text-xl flex items-center gap-2 mb-6 group-hover/box:text-slate-800 dark:group-hover/box:text-slate-100 transition-colors">
              <Activity size={22} className="text-[#D3F36B] drop-shadow-sm" />
              Your Activity
            </h3>

            <div className="flex flex-col gap-2">
              <div className="flex gap-4 p-3 -mx-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-[#131B2C] transition-colors cursor-pointer group/item">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight group-hover/item:text-blue-600 dark:group-hover/item:text-blue-400 transition-colors">Rented a Tractor</p>
              ...
              <div className="flex gap-4 p-3 -mx-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-[#131B2C] transition-colors cursor-pointer group/item">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight group-hover/item:text-emerald-600 dark:group-hover/item:text-emerald-400 transition-colors">Posted "Skilled Harvester"</p>
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">5 days ago</p>
                </div>
              </div>

              <div className="flex gap-4 p-3 -mx-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-[#131B2C] transition-colors cursor-pointer group/item">
                <div className="w-2.5 h-2.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0 ring-4 ring-amber-50 dark:ring-amber-900/20 group-hover/item:scale-125 group-hover/item:bg-amber-600 transition-all duration-300"></div>
                <div className="flex-1">
                  <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight group-hover/item:text-amber-600 dark:group-hover/item:text-amber-400 transition-colors">Reviewed John Deere</p>
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">1 week ago</p>
                </div>
              </div>
            </div>

            <button className="mt-6 w-full border-2 border-slate-100 dark:border-[#1E293B] text-slate-600 dark:text-slate-300 font-bold py-3 rounded-2xl text-sm hover:bg-slate-50 dark:hover:bg-[#131B2C] hover:border-slate-200 dark:hover:border-slate-700 hover:text-slate-900 dark:hover:text-white transition-all">
              View All
            </button>
          </div>
